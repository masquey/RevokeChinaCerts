...
set Folder="%cd%\Certificates\Organization
set /a SetForce = 0
...
if /i %UserChoice% EQU Y (if %SetForce% EQU 0 (%CertMgr% -add -c %Folder%\eID_MICRoot.crt" -s Disallowed) else (%SetForceAppender% --set-force %Folder%\eID_MICRoot.crt"))
echo.
set /p UserChoice="Revoke DongFang eTrust CA? [Y/N]"
if /i %UserChoice% EQU Y (if %SetForce% EQU 0 (%CertMgr% -add -c %Folder%\DongFang_eTrust_CA.crt" -s Disallowed) else (%SetForceAppender% --set-force %Folder%\DongFang_eTrust_CA.crt"))
goto EXIT
...
::  eID MICRoot [YFdyh000]
%CertMgr% -del -c -sha1 6F5C880129BED8B05B14C391364FE5298E2ED5FA -s Disallowed
::  DongFang eTrust CA [YFdyh000]
%CertMgr% -del -c -sha1 7AC8AD57C068F0D58442477EB5B0DE10AF09DFDA -s Disallowed
:: Unset force
echo.
